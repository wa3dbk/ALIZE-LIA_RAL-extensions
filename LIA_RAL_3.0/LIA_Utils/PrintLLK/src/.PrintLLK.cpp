...
int PrintLLK(Config & config)
{
  try{
        ...
        file=config.getParam("inputFileName");
        dir=config.getParam("outputDir");
        fullname=dir+"/"+file+".apost";
        ofstream f_out(fullname.c_str());
        FeatureServer fs(config); 
        
        SegServer segmentsServer;
        LabelServer labelServer;
        fs.init(config,file,labelServer);
        initializeClusters(file,segmentsServer,labelServer,config);
        ...
        ms.loadMixtureGD(config.getParam("inputWorldFilename"));
 
        MixtureGD & UBM=ms.getMixtureGD((unsigned long) 0);
...
for(unsigned long i=0 ; i<_mixsize;i++) f_out<<aPost[i]<<" ";
f_out<<endl;
                        }
if(config.existsParam("addSep"))
f_out<<"#"<<endl;
                }
